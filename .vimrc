...
" Enable syntax highlighting for these languages in markdown files.
let g:markdown_fenced_languages = [ 'cpp', 'sql' ]
...
" set syntax highlighting for 'TODO' files
...
" Search for lines 80 characters or more
nmap <silent>g8 /^.\{80,\}<CR>
...
function! GrepCurrentWord()
    ...
    let command = printf('grep --color=always -rIwn %s $(git rev-parse --show-toplevel)', currentWord)

    exe printf('!echo \$ %s && %s', command, command)
endfunction
command -nargs=0 Grep call GrepCurrentWord()

" Run `black` on the selected code.  Note that we use whichever `black` is
" available from `$PATH`.
command -range=% Black <line1>,<line2>!black --fast --quiet --line-length 79 -
